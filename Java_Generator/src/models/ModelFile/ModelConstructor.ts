...
export default class ModelConstructor{
    ...
    constructor(constrcut_node: ConstructionNode){
      ...
      this.init_nodes = constrcut_node.init_nodes.filter(init => constrcut_node.param_nodes.some(param => param.name === init.name))
      this.composite_nodes = constrcut_node.init_nodes.filter(init => !constrcut_node.param_nodes.some(param => param.name === init.name))
    }
    toString(){
